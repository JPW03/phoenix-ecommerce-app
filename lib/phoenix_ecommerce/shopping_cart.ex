defmodule PhoenixEcommerce.ShoppingCart do
  @moduledoc """
  ...
  def update_cart(%Cart{} = cart, attrs) do
    #Below is the default context generated functionality.
    #It can't handle nested updates to cart items
    # cart
    ...
    # Cast assoc appends a CartItem changeset list to the parent cart changeset
    # If passed to a Ecto.Repo function, Repo handles nested changesets automatically
    # So minimum update code from here:
    # Repo.update(changeset)

    # An Ecto.Multi eseentially encapsulates a chain of transactions in 1 operation
    # If 1 transaction fails, the whole operation fails.
    # We append the update transaction to the Multi with the changeset,
    # Then append delete transactions for all cart items that were updated to quantity 0
    Ecto.Multi.new()
    # :cart and :discarded_items are just the names of the transactions so they can be referenced using other functions
  ...
  alias PhoenixEcommerce.ShoppingCart.CartItem
  ...
  def total_item_price(%CartItem{} = item) do
    ...
    # The Decimal module is used to preserve precision
  end
